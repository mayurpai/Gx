package com.files.io.exception;

public class FileFormatNotSupported extends Exception {

	public FileFormatNotSupported() {
		// TODO Auto-generated constructor stub
	}

	public FileFormatNotSupported(String message) {
		super(message);
		// TODO Auto-generated constructor stub
	}

	public FileFormatNotSupported(Throwable cause) {
		super(cause);
		// TODO Auto-generated constructor stub
	}

	public FileFormatNotSupported(String message, Throwable cause) {
		super(message, cause);
		// TODO Auto-generated constructor stub
	}

	public FileFormatNotSupported(String message, Throwable cause, boolean enableSuppression,
			boolean writableStackTrace) {
		super(message, cause, enableSuppression, writableStackTrace);
		// TODO Auto-generated constructor stub
	}

}
